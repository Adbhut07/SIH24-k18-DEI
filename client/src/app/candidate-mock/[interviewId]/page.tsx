...
export default function InterviewEvaluationChat() {
  ...
  const getAllCandidates = async ()=>{
    try{
      const response = await axios.get(`${process.env.NEXT_PUBLIC_API_ENDPOINT}/api/v1/candidate/getAll`,{withCredentials:true})
      console.log(response?.data)
      console.log(response?.data?.data?.candidates)
      setRole(response?.data?.data?.candidates)
    }
    catch(error){
      console.log(error)
    }
  ...
  return (
    <div className="flex h-screen bg-gray-50 p-4">
      ...
      <Card className="flex-grow flex flex-col max-w-3xl w-full mx-auto shadow-lg">
        <CardContent className="flex flex-col h-full p-4">
          <div className="flex justify-between items-center mb-4">
            <h1 className="text-lg font-semibold">Candidate Answer Evaluation Chat</h1>
            <Button 
      ...
      <div className='flex flex-col gap-2 absolute  left-[200px] top-[85vh]'>
